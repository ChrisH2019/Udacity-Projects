...
def get_token_auth_header():
    ...
    if not auth:  # No authorization found
        raise AuthError({
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:  # No permissions entry found
        abort(400)
    if permission not in payload['permissions']:  # Unauthorized
        abort(403)
    return True
...
def verify_decode_jwt(token):
    ...
    if rsa_key:
        try:
            ...
            raise AuthError({
                'code': 'invalid_claims',
                'desciption': 'Incorrect claims. Please, ' +
                              'check the audient and issuer.'
            }, 401)
